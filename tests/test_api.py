...
def create_mock_config(
    ...
    mock = MagicMock()
    mock.roon.host = roon_host
    mock.roon.port = roon_port
    mock.roon.token = roon_token
    ...
    def test_get_config_returns_safe_values(self, client):
        ...
        with patch("backend.main.get_config") as mock_get_config:
            with patch("backend.main.get_roon_client") as mock_roon:
                ...
                # Should include host but not token
                assert data["roon_host"] == "192.168.1.100"
    ...
    def test_ollama_status_with_custom_url(self, client):
        ...
        with patch("backend.main.get_config") as mock_config:
            with patch("backend.main.get_ollama_status") as mock_status:
                ...
                response = client.get("/api/ollama/status?url=http://custom-server:11434")

                assert response.status_code == 200
                # Verify the custom URL was passed
                mock_status.assert_called_once_with("http://custom-server:11434")
