...
starting work / unsure which track (native vs legacy-docker) -> Read docs/guardrails/PROJECT.md#repository-structure-read-first
working with Roon browse/search/playback semantics -> Read docs/guardrails/PROJECT.md#roon-api-constraints
Library-first (product constitution): every suggested track must exist either in the user's Roon library or on Qobuz — nothing is hallucinated.
Native is the primary product — Swift packages native/RoonSage (macOS 14 / iOS 17) + native/RoonProtocol; no Docker/Python in this path.
test -> cd native/RoonProtocol && swift test ; cd native/RoonSage && swift test
before tagging / release-sanity (CI is stricter than debug) -> cd native/RoonSage && swift build -c release --product RoonSage
lint gate -> swiftlint lint --config .swiftlint.yml
iOS build -> cd native/iosapp && xcodegen generate (regenerates RoonSageiOS.xcodeproj — never hand-edit it)
release DMGs -> native/scripts/build-release.sh [version] / build-analyzer-release.sh [version]
tag namespaces: app vX.Y.Z · iOS ios-vX.Y.Z · analyzer analyzer-vX.Y.Z (scripts match their own prefix; unfiltered git describe stamps the wrong version)
releasing / signing / deploy-to-mini / ports / ZeroTier -> Read docs/guardrails/PROJECT.md#native-build-release
