...
## Now
B1-B6a + B7a + B8-features KLAAR. Terugblik-consumers (op-deze-dag + taste-timemachine) nu in de app (Recent-scherm). Live op mini = analyzer-v1.1.108 (endpoints live). Volgende: NL-steering-consumer (tekstveld/voice) óf meer B8-features.

## Next
- Resterende consumer: steerActiveRadio(phrase:) → tekstveld/voice-hook op het radioscherm (UI)
- B8+ Features resterend (logica-zwaar/testbaar eerst): loudness-normalisatie, crossfade, scenes, share-cards, Siri-intents, Control Center, gapless, CarPlay, chat-agent
