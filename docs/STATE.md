...
## Now
B1-B6a + B7a + B8(skip-re-steer, op-deze-dag, taste-timemachine, NL-steering) KLAAR. Live op mini = analyzer-v1.1.108; NL-steering (v1.1.109) is client-side radio-logica, geen live consumer → niet gedeployd. Volgende: meer B8-features óf de UI/Siri/chat-consumers die de nieuwe API's zichtbaar maken.

## Next
- B8+ Features resterend (kies logica-zware/testbare eerst): loudness-normalisatie, crossfade, scenes, share-cards, Siri-intents, Control Center, gapless, CarPlay, chat-agent
- Consumers voor gebouwde backend-features (UI/Siri/endpoint): /on-this-day + /taste-timemachine (client-fetch+view), steerActiveRadio(phrase:) (voice/chat/tekstveld op radioscherm)
