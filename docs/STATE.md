...
## Now
B1-B5 KLAAR + analyzer-v1.1.101 op de mini. B6a (ZoneGate library play-now → active-output) GESHIPT. Rest B6 = grotendeels GUI-visueel (niet headless verifieerbaar). Daarna B7 architectuur, B8+ features.

## Next
- B6b (optioneel, per-verb, hoger risico): MusicMapView/AskView/CustomRadioView/DiscoverWeeklyView play-knoppen honoreren lokale output — elk gebruikt een ándere verb (playTrack(id:) etc.), lokale routing per verb vereist + geen GUI-verificatie op mini
- B7 Architectuur: RoonClient sub-coördinatoren + mock-transport testharnas (testbaar)
