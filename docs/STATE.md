...
## Now
B1-B6a KLAAR + analyzer-v1.1.101 op de mini. B7a (mock-transport testharnas + eerste TransportService-tests) GESHIPT. Volgende: B7b RoonClient sub-coördinatoren (grote refactor, hoog risico, alleen build-verifieerbaar zonder live Roon) óf B8+ features óf B6b.

## Next
- B7b Architectuur (groot/risico): RoonClient (900r god-object) opsplitsen in sub-coördinatoren — hoge blast radius op de live-connectieflow, niet headless verifieerbaar zonder Roon Core. Overweeg incrementeel + build-gated
- B8+ Features (13): skip re-steer, loudness, crossfade, Siri-intents, Control Center, NL steering, share-cards, "op deze dag", gapless, taste-timemachine, scenes, CarPlay, chat-agent (kies testbare/logica-zware eerst)
