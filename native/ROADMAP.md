...
- [~] **E5. Sonic embeddings (CLAP via Core ML).** Lift the analyzer from 7 scalars to a learned 512-dim embedding per track + mood scores, so Similar/Map/Path/Alchemy/Fingerprint run on a real vector index (k-NN) instead of rule-based BPM/Camelot/tag distance. Model: `laion/larger_clap_music_and_speech`. See `docs/EMBEDDING_NOTES.md`.
  ...
  - [ ] **E5c. Persist + serve.** Extend `AudioFeatures` (embedding/moods/version) + `FeatureStore` (BLOB columns, `ALTER TABLE` adds), serve over `/features` (base64) + a compact binary `/embeddings`.
  - [ ] **E5d. Vector index + rewire.** `VectorIndex` (brute-force cosine via Accelerate) behind `SonicLibraryCache`; rewire Similar/Fingerprint/Path/Alchemy onto it; Music Map → PCA-2D; text search → `textEmbedding` (needs a Swift CLAP tokenizer).
  - **OPEN:** how to ship the `.mlpackage` weights (Git LFS / build-time download) — currently git-ignored.
