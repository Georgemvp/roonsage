...
- [ ] **F9. Swipe-stack polish (1b).** Full-swipe Overslaan restored behind an **undo** affordance — delayed-commit (hold the reject POST ~4 s, surface "Overslaan ongedaan maken", cancel the pending Task on undo; no new endpoint) — plus a first-run onboarding empty-state that explains Ontdekkingen (outward) vs. Ontdek (library-only).
