...
user's library. Powers harmonic-mixing DJ-set generation, audio-aware filter
parameters (`bpm_min`, `bpm_max`, `energy_min`, `camelot_keys`), and the
sonic-clustering / Music Map visualizations.
...
Submodules:
  ...
  clustering    — UMAP + HDBSCAN over the feature vectors (Music Map).
"""
